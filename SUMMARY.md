...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 OPC Server             ┌──────────┐    ┌──────────┐  ┌──────────┐  ┌─────────┐
 (<OPC_SERVER_IP>)          │          │    │ Telegraf │→│ InfluxDB │→│ Grafana │
      │                 │          │    └──────────┘  └──────────┘  └─────────┘
      ▼                 │          │
 ┌──────────┐           │  Kafka   │    ┌──────────┐  ┌──────────┐  ┌─────────┐
...
|-------|--------|-----------|
| **OPC Server** | แหล่งข้อมูล sensor | อุปกรณ์ OT/SCADA ที่เปิดข้อมูลผ่าน OPC-UA protocol เช่น PLC, DCS, RTU — ในระบบนี้ใช้ simulator ที่ `<OPC_SERVER_IP>` ส่งค่า 12 sensors ทุก 2 วินาที |
...
**Components:**
- **NiFi Edge** — รับข้อมูลจาก OPC server `<OPC_SERVER_IP>`, ส่งไป Kafka (heap 1G–2G)
...
kubectl exec -n it deploy/influxdb -- influx query \
  --token CHANGE_ME --org mintpower-org \
  'from(bucket:"opc-data") |> range(start:-1m) |> filter(fn:(r)=>r._measurement=="opc_data") |> count()'

# Grafana dashboard
# http://<K3S_NODE_IP>:30300/d/ffl0uchin1hxcc/opc-sensor-dashboard
```
...
|---------|-------------|----------|-----------------|---------|
| **Grafana** | `:30300` · grafana.mintpower.local | `admin` | `CHANGE_ME` | |
| **InfluxDB** | `:30086` · influxdb.mintpower.local | `admin` | `CHANGE_ME` | Org: `mintpower-org` · Bucket: `opc-data` |
| **InfluxDB Token** | API / Telegraf | — | `CHANGE_ME` | ใช้ใน Telegraf config |
| **MinIO** | `:30901` · minio.mintpower.local | `minioadmin` | `CHANGE_ME` | Bucket: `opc-raw` |
| **NiFi Core** | `:31443` (HTTPS) · nifi-core.mintpower.local | `admin` | `CHANGE_ME` | |
| **NiFi Edge** | `:31444` (HTTPS) · nifi-edge.mintpower.local | `admin` | `CHANGE_ME` | |
| **OpenMAINT** | `:30885` · openmaint.mintpower.local | `admin` | `admin` | CMDBuild default — เปลี่ยนหลัง deploy จริง |
| **PostgreSQL** (superuser) | internal only | `postgres` | `CHANGE_ME` | สำหรับ maintenance เท่านั้น |
| **PostgreSQL** (openmaint) | internal only | `openmaint` | `CHANGE_ME` | DB: `openmaint` |
...
```
URL:  https://<K3S_NODE_IP>:31444/nifi  (admin / CHANGE_ME)
...
```
URL:  https://<K3S_NODE_IP>:31443/nifi  (admin / CHANGE_ME)
...
| Grafana OPC Dashboard | ✅ พร้อม | 12 panels, refresh 5s — `http://<K3S_NODE_IP>:30300/d/ffl0uchin1hxcc` |
...
| OpenMAINT | ✅ READY | `http://<K3S_NODE_IP>:30885/cmdbuild` — admin/admin |
...
```
URL:  https://<K3S_NODE_IP>:31444/nifi
...
1. เปิด `https://<K3S_NODE_IP>:31444/nifi` → login `admin / CHANGE_ME`
...
InfluxDB UI: `http://<K3S_NODE_IP>:30086`
```flux
...
```
URL:  https://<K3S_NODE_IP>:31443/nifi
User: admin / CHANGE_ME
```
...
เปิด `https://<K3S_NODE_IP>:31443/nifi` → login `admin / CHANGE_ME`
...
| 4 | `AWSCredentialsProviderControllerService` | MinIO-Credentials | `Access Key` = `minioadmin`, `Secret Key` = `CHANGE_ME` |
...
```
URL: http://<K3S_NODE_IP>:30800  (ไม่มี auth)
```
...
```
URL:  http://<K3S_NODE_IP>:30885/cmdbuild
User: admin / admin
```
