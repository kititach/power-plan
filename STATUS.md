...
## 🟢 สถานะล่าสุด — 2026-05-14 (Downsampling pipeline + Presentation assets)
...
Prosys OPC UA (mintserver:53530)
  └─[opc.tcp]─► NiFi Edge (Groovy/Eclipse Milo) ──► Kafka: opc-raw-data
                                                          ...
                                                          ├─► NiFi Core ──► MinIO (opc-raw, JSON)            ✅
                                                          │                  └─► Trino CronJob ──► opc-1h    ✅
                                                          │                                   └─► opc-1d    ✅
...
| Trino | ✅ Running | catalog `minio`, tables: sensor_data_flat / sensor_1h / sensor_1d |
| **Downsample CronJob** | ✅ Active | `downsample-1h` (ทุกชม.) + `downsample-1d` (ทุกวัน 07:10) |
...
- [x] **Downsampling pipeline** — 2026-05-14 (Trino CronJob 1h/1d, MinIO opc-1h/opc-1d)
- [x] **Presentation assets** — 2026-05-14 (Block diagram, Scale-up plan, Dataflow diagram)
...
| **MinIO** | http://10.85.3.104:30901 | minioadmin | Minio@mintpower2024 |
...
| `plan/scale-presentation.md` | Scale-up plan 3 ระดับ (100/1k/10k tag/s) สำหรับนำเสนอ |
| `plan/scale-10k-tags-plan.md` | แผน scale ระบบรองรับ 10,000 tags/s + retention 3เดือน/5ปี |
| `plan/migrate-to-new-machine.md` | แผนย้ายระบบไปเครื่องใหม่ (offline/no git) |
| `scripts/gen-downsample-sql.py` | Generate Trino SQL downsampling จาก 307 OPC tags |
| `manifests/phase4/downsample/` | Trino setup SQL, ConfigMap, CronJobs (downsample-1h/1d) |
| `concept/diagrams/diagram-a-with-ot.svg` | Block diagram ภาพรวมระบบ (สำหรับนำเสนอ) |
| `concept/diagrams/diagram-scale.svg` | Scale-up plan diagram 3 ระดับ (สำหรับนำเสนอ) |
| `concept/diagrams/diagram-dataflow.svg` | Data format ที่แต่ละ block ในระบบ (สำหรับนำเสนอ) |
...
### 2026-05-14 — Downsampling Pipeline + Presentation Assets

#### Downsampling CronJob ✅
- MinIO buckets `opc-1h` และ `opc-1d` สร้างแล้ว
- Trino tables:
  - `minio.opc.sensor_data_flat` — 307 double columns อ่านจาก `opc-raw/data/` (flat JSON)
  - `minio.opc_agg.sensor_1h` — Parquet, tall format (tag_name, avg/max/min/samples), partitioned year/month/day
  - `minio.opc_agg.sensor_1d` — Parquet, aggregate จาก sensor_1h
- CronJob `downsample-1h` (`5 * * * *`) — aggregate previous hour ด้วย CROSS JOIN UNNEST 307 tags
- CronJob `downsample-1d` (`10 0 * * *`) — aggregate previous day จาก sensor_1h
- ทดสอบ: INSERT 307 rows/hour ✅
- ไฟล์: `manifests/phase4/downsample/` (ConfigMap, CronJobs, SQL), `scripts/gen-downsample-sql.py`

#### Presentation Assets ✅
- `concept/diagrams/diagram-a-with-ot.svg` — Block diagram (OT Zone + k8s + Host services)
- `concept/diagrams/diagram-scale.svg` — Scale-up plan 3 ระดับ (100/1k/10k tag/s)
- `concept/diagrams/diagram-dataflow.svg` — Data format จริงที่แต่ละ block

#### Bug Fixed
- MinIO password ใน `minio-secret.yaml` แก้ `CHANGE_ME` → `Minio@mintpower2024` (password จริงในคลัสเตอร์)
